...
async function main(): Promise<void> {
  ...
  if (typeof sessionId === 'string' && sessionId) {
    const body: Record<string, unknown> = { sessionId, pid: process.ppid, cwd: input.cwd };
    if (phase === 'start') {
  ...
  if (phase === 'start') {
    ...
    let guidance = '';
    try {
      const summary: any = await (await fetch(`${baseUrl()}/api/summary`)).json();
    ...
    process.stdout.write(
      JSON.stringify({
        hookSpecificOutput: {
          ...
          additionalContext:
            `herbert analytics ${ensured === 'started' ? 'server started (this session is the server)' : 'server already running (this session is a client)'}; dashboard at ${baseUrl()}. ` +
