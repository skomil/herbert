...
export interface MetricRecord {
  kind: 'metric';
  t: number; // ms epoch
  /** startTimeUnixNano — distinguishes cumulative series across process restarts */
  start?: string;
  name: string;
  value: number;
...
export type OtelRecord = MetricRecord | EventRecord;

type AnyObj = Record<string, any>;

function attrValue(v: AnyObj | undefined): string | undefined {
...
function attrsToObject(list: AnyObj[] | undefined): Record<string, string> {
  const out: Record<string, string> = {};
  for (const kv of list ?? []) {
    const val = attrValue(kv.value);
    if (typeof kv.key === 'string' && val !== undefined) out[kv.key] = val;
  }
  return out;
}
...
export function parseMetrics(payload: AnyObj): MetricRecord[] {
  ...
  for (const rm of payload?.resourceMetrics ?? []) {
    ...
    for (const sm of rm.scopeMetrics ?? []) {
      for (const m of sm.metrics ?? []) {
        const sum = m.sum;
        const gauge = m.gauge;
        const dataPoints: AnyObj[] = sum?.dataPoints ?? gauge?.dataPoints ?? [];
        const temporality: MetricRecord['temporality'] = sum
...
export function parseLogs(payload: AnyObj): EventRecord[] {
