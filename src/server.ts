...
const EVENT_TYPES: SummaryEventType[] = ['specification', 'correction', 'retro'];
...
function usageCsv(summary: ReturnType<Store['summary']>): string {
  const iso = (t?: number) => (t ? new Date(t).toISOString() : '');
  const rows = summary.sessions.map((s) => {
    const tok = (k: string) => s.tokens[k] ?? 0;
...
export function startServer(p: number = port(), dir: string = dataDir()): Promise<RunningServer> {
      ...
      switch (route) {
        ...
        case 'POST /api/prd/import': {
          ...
          if (body.summary !== undefined && body.summary !== null) {
            if (typeof body.summary !== 'string') return json(res, 400, { error: 'summary must be a string' });
          ...
          if (body.components !== undefined) {
            ...
            for (const [k, v] of Object.entries(body.components)) {
              if (typeof v !== 'string') return json(res, 400, { error: `component "${k}" must map to a markdown string` });
              docs.push([k, v]);
            }
          }
          const existing = store.prd(repo);
          const has = (k: string) => (k === '' ? !!existing.summary : k in existing.components);
          let imported = 0;
          for (const [k, v] of docs) {
            if (v.length > MAX_PRD_CHARS) {
              return json(res, 400, { error: `doc "${k || 'summary'}" exceeds ${MAX_PRD_CHARS} characters` });
            }
            if (fill && has(k)) continue;
          ...
          if (body.specifications !== undefined) {
            ...
            for (const s of body.specifications) if (store.importSpec(s, repo)) importedSpecs++;
          }
        ...
        case 'POST /api/report-window': {
          ...
          for (const [k, lo, hi] of [['weekday', 0, 6], ['hour', 0, 23], ['utcOffset', -12, 14]] as
        ...
        case 'GET /api/report.csv': {
          ...
          res.writeHead(200, {
            ...
            'content-disposition':
              `attachment; filename="herbert-usage-${new Date(from).toISOString().slice(0, 10)}.csv"`,
          });
        ...
        case 'POST /api/events': {
          ...
          if (body.type === 'retro') {
            for (const k of ['whatWorked', 'whatDidnt', 'changeNext']) {
              ...
              if (bad) return json(res, 400, { error: `${k}: ${bad}` });
              sections[k] = body[k];
            }
          }
          if (body.type === 'specification' && body.context !== undefined) {
            const bad = validateSummary(body.context);
            if (bad) return json(res, 400, { error: `context: ${bad}` });
            sections.context = body.context;
          }
          let sessionId: string | null = body.sessionId ?? null;
          let repo: string | null = typeof body.cwd === 'string' && body.cwd ? body.cwd : null;
        ...
        case 'POST /api/specs/feedback': {
          ...
          const feedback = (body.feedback || null) as SpecFeedback | null;
        ...
        case 'POST /api/specs/annotate': {
          ...
          if (body.revision !== undefined) {
            ...
            if (ann.revision) ann.status = 'proposed'; // a revision reopens the spec
          }
          ...
          if (body.summary !== undefined) {
            ...
            if (bad) return json(res, 400, { error: `summary: ${bad}` });
            ann.summary = body.summary;
          }
          if (body.deleted !== undefined) ann.deleted = body.deleted === true;
          if (body.status !== undefined) {
          ...
          if (ann.status !== undefined && ann.status !== 'in_progress') ann.stage = null;
          ...
          if (body.deps !== undefined) {
            if (!Array.isArray(body.deps) || body.deps.some((x: unknown) => typeof x !== 'string')) {
              return json(res, 400, { error: 'deps must be an array of component names' });
            }
            const deps = body.deps.map((x: string) => x.trim()).filter(Boolean);
            if (deps.length > 20) return json(res, 400, { error: 'at most 20 deps per spec' });
        ...
        case 'POST /api/preview': {
          ...
          if (raw.length > 2048) return json(res, 400, { error: 'url is too long' });
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(p, host(), () => {
      ...
      resolve({
        ...
        store,
        port: p,
        close: () => new Promise<void>((r, x) => server.close((e) => (e ? x(e) : r()))),
      });
    });
  });
}
...
if (isMain) {
  startServer()
    .then((running) => {
      console.log(
        `herbert server listening on http://${host()}:${running.port} (data: ${dataDir()})`,
      );
    })
    .catch(async (err: any) => {
    if (err?.code === 'EADDRINUSE') {
      // Lost the leader race (or a server is already up): become a client.
      if (await isHerbertUp()) process.exit(0);
